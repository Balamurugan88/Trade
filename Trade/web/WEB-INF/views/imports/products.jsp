    ...
    <body>
        <div class="col-sm-12">
            <div class="col-sm-12">
                ...
                <form method="post" action="<%=request.getContextPath()%>/admin/ExcelUpload?uploadType=production&type=1&category=<%=request.getParameter("category")%>" enctype="multipart/form-data">
                    ...
                    <tbody>
                        <c:forEach items="${prodList}" var="prod">
                            <tr>
                                ...
                                <td><fmt:formatNumber type="number" value="${prod.value}"/></td>
                                <td><fmt:formatNumber type="number" value="${prod.quantity}"/></td>
                                <td><c:out value="${prod.year}"/></td>
                                 <%if(CustomUtils.isAdmin(request)){%>
                                <td>
                                    
                                    <a href="<%=request.getContextPath()%>/admin/import/delete?id=${prod.id}&type=production&category=<%=request.getParameter("category")%>"
